...
def run_next_experiment():
	...
	queue_data["last_updated"] = datetime.now(timezone.utc).astimezone().isoformat()
				...
				for line in tf:
					...
					if line_data.get("type") == "epoch":
						...
						if os.path.exists(exp_config_path):
							with open(exp_config_path, "r", encoding="utf-8") as cf:
...
*   **Nombre**: {task_name}
*   **Fecha/Hora**: {datetime.now(timezone.utc).astimezone().strftime('%Y-%m-%d %H:%M:%S %Z')}
...
"""
	...
	queue_data["last_updated"] = datetime.now(timezone.utc).astimezone().isoformat()
	save_queue(queue_path, queue_data)
	print(f"✅ Cola de experimentos actualizada. Siguiente tarea pendiente disponible.")
