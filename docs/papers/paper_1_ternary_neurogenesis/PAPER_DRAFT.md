...
### 1.1 Positioning: Why Bit?

A fair question deserves a direct answer: *why grow a 2.4M-parameter ternary organism when Qwen3-0.6B already runs at 96 tok/s on the same 2W NPU used in our testbed?* Because Bit does not compete on tasks — Bit is not an engine, it is an **observable organism**. Our claims occupy a segment that frontier and small open models structurally cannot serve, for three reasons:

1. **Interpretability of growth, not of behavior.** Every weight in Bit is auditable and every architectural change is event-logged: we can watch *how* a capability appears (which pain signal triggered growth, which curriculum stage unlocked composition). A pretrained model — however small — arrives as a fossil: capable, but with its developmental history erased. Bit is a microscope on the process that scale-first research treats as a black box, and large laboratories have no incentive to build it, because their economics reward selling scale, not understanding growth.

2. **Sovereignty from the first token.** Bit inherits no upstream weights, no undisclosed training corpus, no third-party license or alignment residue. For deployments where provenance is a hard requirement (personal sovereign agents, regulated edge devices), "small and borrowed" is not equivalent to "small and owned".

3. **Radical frugality.** At ~586KB, Bit targets the always-on sentinel niche (2W, embedded, battery-first) where even a 0.6B model is an order of magnitude too heavy.

Beyond these engineering claims lies the hypothesis that motivates the project: **everyday AI does not need graduate physics.** The overwhelming majority of daily human-AI interaction requires language, common ground, and calibrated honesty — not encyclopedic knowledge. A modest mind that *knows what it does not know* (the metacognitive verification loop of §3.3 exists precisely for this) may serve most everyday companion roles, and may equally serve as the lingua franca of **machine-to-machine communication**, where two tiny agents coordinating through a shared vocabulary (see our companion work on intra-forward distributed dispatch) need mutual intelligibility, not brilliance. We do not claim this hypothesis is proven — the roadmap below is the experiment designed to test it.

**The staged bet.** Our sequencing is deliberate. The hardware MoE layer (Phase A) is parked until Bit completes the developmental curriculum through the 8-year-old milestone. Then we re-instantiate the survival agents — Nico, Sofy, and a third sibling, Hugo — *from Bit's base brain*: agents that know nothing of survival but arrive with language and comprehension, released into the arena to test whether general developmental competence transfers to embodied evolution. If agents that can understand and communicate outlearn agents that were shaped by survival alone, the thesis holds: **cognition does not require scale; it requires structure — curriculum, pressure, and the ability to grow.**
