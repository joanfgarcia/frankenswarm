...
def check_lifecycle(expert_id, current_fitness):
    ...
    if query_action:
        ...
        if accion == "congelar":
            print(
                f"🧠 [Prolog MoE]: {expert_id} alcanzó fitness {current_fitness}. ¡Congelando pesos!"
            )
            prolog.retractall(f"experto({expert_id}, _, _, _)")
            prolog.assertz(
                f"experto({expert_id}, {capacidad}, {current_fitness}, congelado)"
            )
            # Aquí disparas tu script de PyTorch para guardar/congelar pesos
        else:
            print(
                f"🧬 [Prolog NEAT]: {expert_id} sigue en bucle de mutación. Fitness insuficiente."
            )
