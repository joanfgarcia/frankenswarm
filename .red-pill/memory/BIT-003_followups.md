...
- **DL-023 pipeline Bit + intención**: input→intérprete L→K-65P (caché)→Bit MoE→K-65P out (caché)→intérprete→output. Traducción=INTERPRETACIÓN (frase-idea, nunca palabra aislada; 'get' solo-en-contexto). El intérprete = proyecto aparte (LLM por lengua; la cata curada es su bootstrap). Dos memorias: SSM=hipocampo, caché K-65P=córtex auditable (KB hechos + log episódico). Marcador `[Q cláusula]` con glifo querer+saber — la fuerza ilocutiva (curiosidad/duda/sorpresa) es del intérprete. Ambigüedad interactiva: want-know + SILENCIO permiten preguntar hacia atrás.
