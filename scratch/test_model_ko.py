...
def load_m(path):
	...
	if os.path.exists(path):
		...
		# Resize checkpoints if different
	...
	# Ensure COOP_N_ACTIONS is preserved
...
world = CooperativeWorld(seed=42, hunger_rate=1.5)
state_a, state_b, state_c = world.reset(seed=42)

for tick in range(1, 150):
	# Choose actions using model
	...
	with torch.no_grad():
		...
		probs_a = get_masked_probs(logits_a, 1, 1, True, mask_a)
		...
		probs_b = get_masked_probs(logits_b, 1, 1, True, mask_b)
		...
		probs_c = get_masked_probs(logits_c, 1, 1, True, mask_c)
		action_c = COOP_ACTIONS[torch.argmax(probs_c, dim=-1).item()]

	res_a, res_b, res_c, info = world.step(action_a, action_b, action_c)
	
	print(f"Tick {tick:03d} | A ({action_a}): H: {state_a.hambre:.1f}, S: {state_a.sed:.1f}, KO: {state_a.debuff_inconsciente_ticks} | B ({action_b}): H: {state_b.hambre:.1f}, S: {state_b.sed:.1f}, KO: {state_b.debuff_inconsciente_ticks} | C ({action_c}): H: {state_c.hambre:.1f}, S: {state_c.sed:.1f}, KO: {state_c.debuff_inconsciente_ticks}")
	
	is_any_ko = state_a.debuff_inconsciente_ticks > 0 or state_b.debuff_inconsciente_ticks > 0 or state_c.debuff_inconsciente_ticks > 0
	if is_any_ko:
		print("  --> Someone is unconscious!")
