...
def test_seeds():
	...
	for seed in range(100):
		...
		ticks = 0
		shouts = 0
		while state_a.alive and state_b.alive and state_c.alive and ticks < 200:
			...
			res_a, res_b, res_c, w_info = world.step(action_a, action_b, action_c)
			if res_a.get("shouted") or res_b.get("shouted") or res_c.get("shouted"):
				shouts += 1
			ticks += 1
	...
	print("Top 10 Seeds by Survival:")
	for seed, ticks, shouts in results[:10]:
		print(f"Seed {seed:03d} | Survival: {ticks:3d} ticks | Shouts: {shouts:2d}")
