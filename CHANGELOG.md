...
- **[🔴 RETRACTACIÓN] "El glyph suspendió 2_years a 128d" no está demostrado**: el suspenso fue un **crash de infraestructura** (Samantha devolvió `None` + timeout del offload, log `dd078baf` líneas 530-538), no una calificación — la neurogénesis 128→256 (la "primera ejercición de la escalera") se disparó sin examen real, y el checkpoint 128d pre-crecimiento no sobrevivió. **[FIX] Contrato de exit-codes evaluador↔state_manager**: `0` aprobado · `2` suspenso CALIFICADO · cualquier otro rc (incl. `1` por excepción) = infraestructura → reintento único y, si persiste, pausa del operador (`eval_infra_error`) SIN contar suspenso ni disparar neurogénesis. 4 tests nuevos (8/8 en `test_exam_failure_policy.py`). Receta `bit003_glyph_v41_2y_x1` lista para re-responder la pregunta abierta (¿pasa el glyph 2_years a 128d bajo ×1?) con examen real.
- **[🔴 RETRACTACIÓN] La "curva riesgo-cobertura" del 31-ago era un artefacto**: `run_battery_v3.py` nunca calculaba la confianza (`conf = None` desde su nacimiento) — el sort estable con clave constante ordenaba vistas→no vistas, y el "78% en el 20% más seguro vs 19.4% medio" era la accuracy de las primeras 50 preguntas VISTAS contra la mezcla. Con la confianza REAL (softmax post-máscara del primer token): dentro de lo visto la calibración existe y es fuerte (top-20% → 90-100% de acierto en los 6 checkpoints); en lo no visto es casi nula (top-20% → 0-5.4%). "Sé lo que sé" es real; "sé lo que no sé" ante lo nuevo NO está demostrado.
- **[FIX] Runner de batería re-tirado sobre los 6 checkpoints** (`run_battery_v3.py`): confianza real, contexto tokenizado con `tokenize()` (antes `q.split()` metía `<unk>` en 23/195 no-vistas — 19/38 de aritmética, medio estrato saboteado por el instrumento), dedupe de (q,a) exactos al cargar (58→56 vistas, 195→189 no-vistas), retención y procedencia completa (md5s, set_id, git rev, timestamp) persistidas en el JSON, máscara del directorio del checkpoint evaluado, curvas riesgo-cobertura separadas (mixta/vistas/no-vistas). Resultados honestos abajo (sección de la confrontación, corregida en sitio).
- **[FIX] El check in-gate de los banks nació muerto** (`generate_exam_banks.py`: `gate_idx` local a `None`) — dejó entrar 3 respuestas inaprendibles bajo máscara (`burns`, `howl`, `hiss`). Cableado y regenerado: 442 → **439 preguntas**.
- **[FIX] Batería v2 congelada** (`configs/battery_v3/age4_v2.json`, `set_id age4_v2_*`): los banks DL-013 entran al universo entrenado — **28/195 "no vistas" del set v1 colisionaban con los banks** y habrían inflado la cognición al cablearlos. Además: dedupe por pregunta (una q con dos golds garantiza un fallo estructural — fuera `if i touch the fire → burns/hurt`), fuera las 2 vistas sin verificar presencia, y retirados 2 riddles con gold factualmente incorrecto (`it says caw → owl`, `it says croak → duck`). Set v2: **51 vistas (todas verificadas) / 169 no-vistas**. El v1 queda congelado para la confrontación v4 en curso; v2 rige en cuanto los banks se cableen.
- **[FIX] Señales de hito por brazo**: `milestone_achieved/failed.json` eran globales y los brazos en serie se las pisaban (el 3_years del standard sobrescribió la del glyph); ahora se escriben también en el `state_dir` del brazo.
- **[FIX] Módulos DL-014**: `k65p_dictionary.upsert_word` devolvía `lastrowid` inválido tras UPDATE (las surfaces ES podían colgar del id equivocado; la BD actual está limpia — 0 surfaces) y pisaba estados curados (`canonical/molecule/prime/explicated`) desde lotes automáticos — guard con `force`; `upsert_phrase` no actualizaba `source` y devolvía id inválido. `nsm_explication` se tragaba typos de primos en silencio (justo el fallo del que decía proteger) — ahora raja siempre. `en_lexicon`: la forma canónica gana a los alias (estaba invertido), el primo 35 (`moment`) recupera su superficie (`a moment`/34 se la robaba al partirse en tokens), `was/were/been/being` → primo 25 (estaban en `DROP_WORDS` y en el léxico a la vez), y `a` deja de ser alias ambiguo 52/55. 65/65 primos con superficie.
- **[VERIFIED] No-bugs descartados en la auditoría**: `best_stage_val_loss`/`epochs_since_best` del standard son coherentes (semántica `min_delta` de la paciencia + orden de guardado); las 18 épocas del glyph más allá de `completion` son el grano del step (20 épocas) del job runner; el examen de 4_years del glyph auto-calificado 10/10 es la rama legítima del auto-grader (10/10 exact-match, sin necesidad de LLM).

### 🔬 BIT-003 — primera confrontación honesta: glyph vs standard ×1 + el expediente de tres instrumentos (2026-08-31/09-01) — **números corregidos el 09-01 tras la auditoría**

- **[RESULT corregido] La confrontación sigue ABIERTA**: el standard aprobó 2 y 3 años a 128d sin crecer; el glyph corre a 256d por una neurogénesis disparada por infra (ver retractación arriba) y su hito de 4 años ya está aprobado; el standard aún no ha llegado a 4_years — no hay comparación homóloga final. Batería congelada v1 re-tirada (56 vistas / 189 no-vistas, runner corregido): glyph 2y (256d) gate 44.6% / cog 1.6% / retención 19.2%; glyph 4y (256d) gate 69.6% / cog 4.2% / retención 30.8%; standard 2y (128d) gate 51.8% / cog 5.8% / retención 25.6%; standard 3y (128d) gate 67.9% / cog 3.2% / **retención 42.3%** (la más alta medida). Las diferencias de cognición (1.6-5.8%, n=189) están dentro del ruido; lo sólido es que **ninguna representación generaliza a esta escala** — y que ambos brazos dieron respuestas token-idénticas en los exámenes de hito de 2y/3y (mismos errores, mismas notas): el examen de hito mide el corpus, no la representación.
- **[NEW] Matriz de puntuación 3×2 del operador**: visto +1/+1/−1 (alucinar lo enseñado es el pecado; el "no sé" honesto premia) · no visto +2/0/0 (generalizar es el premio doble, fallar no castiga). La ignorancia total honesta (+56) supera al mejor Bit actual (+22/+16): el gradiente empuja a aprender, no a alucinar. Abstenciones medidas: 0 en los 6 checkpoints — el argmax forzado impide el canal de silencio ("i dont know" está en vocab y gateos, verificado).
- **[NEW] Gate de retención** (persistido en el JSON desde el 09-01): 80 frases aleatorias del pool de etapa, cloze determinista contra la palabra real del corpus — cero curación. glyph 2y 19.2% · std 2y 25.6% · std 3y 42.3% · glyph 4y 30.8% · glyph v3 ×10 4y 24.4% · glyph ×300 v2 4y 23.1%.
- **[PROFILE corregido] Por estratos (no vistas, runner sin `<unk>`)**: interrogativo y riddles a **0/39 en los 6 checkpoints sin excepción**; cruce 1-4/40; aritmética 0-6/33 (mejor: standard 2y); predicado 0-3/38. La generalización composicional está a cero en cualquier representación a esta escala — este hallazgo de la versión original SOBREVIVE a la auditoría y es el resultado central de la confrontación.
