...
class TurboQuantCache:
	...
	def __init__(self, max_seq_len: int, head_dim: int, num_heads: int):
		...
		self.k_cache_quantized = torch.zeros((num_heads, max_seq_len, head_dim), dtype=torch.int8)
		self.v_cache_quantized = torch.zeros((num_heads, max_seq_len, head_dim), dtype=torch.int8)
