...
def load_config() -> dict:
	parser = argparse.ArgumentParser(description="Arena de Generalización Aritmética")
...
def run_math_generalization_arena():
	config = load_config()
	experiment_id = config["experiment_id"]
	print(f"=== 🧮 Iniciando Arena de Generalización Aritmética — {experiment_id} ===")
	...
	for epoch in range(epochs):
			...
			for idx_spk in range(pop_size):
				for idx_lis in range(pop_size):
					...
					for a, op, b, r in test_eqs:
						...
						# Generar mensaje discreto (temperatura baja 0.1 para evitar ruido aleatorio)
		...
		print(f"🧪 [Test Generalización] Consenso Aritmético: {acc_joint_test:.2f}% ({model_eval_correct}/{model_eval_total})")
		...
		# Registrar telemetría (acc_homeostasis mapea a acc_joint_test para preservar el formato)
		logger.log_epoch(
			epoch=epoch + 1,
			loss_avg=avg_loss,
			acc_concept=acc_a,
		...
		# Detención temprana basada en generalización real (test set)
		if epoch >= transition_end and acc_joint_test >= 75.0:
			print(f"\n🏆 ¡HIT DE GENERALIZACIÓN ALCANZADO! Consenso en test set: {acc_joint_test:.2f}%.")
			logger.log_event("promotion", {"grade": 1, "acc_joint_test": acc_joint_test})
			break
	...
	print(f"💾 Checkpoint del mejor agente de generalización guardado en {checkpoint_save_path}")

	logger.close()


if __name__ == "__main__":
	run_math_generalization_arena()
