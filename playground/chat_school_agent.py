...
def sample_next_token(logits, temperature=0.7, repetition_penalty=1.2, generated_tokens=None, word_to_idx=None):
	...
	# Filter special tokens from being generated if possible (except <pad> which is our stop token)
...
def run_chat(args):
	...
	while True:
		try:
			...
			# Generar la predicción autoregresiva de múltiples tokens
			generated_tokens = []
			with torch.no_grad():
				for _ in range(args.max_tokens):
					...
					# sample next token using penalty and temperature
					...
					# Stop on pad, unk or special stop tokens
...
if __name__ == "__main__":
	...
	parser.add_argument("--max_tokens", type=int, default=15, help="Número máximo de tokens a generar (default: 15)")
