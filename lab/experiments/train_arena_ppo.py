...
from src.bitnet.growth.net2net import grow_action_head, net2wider_linear
...
def run_arena_ppo_training():
	...
	if dojo_pretrain_steps > 0:
		...
		from src.bitnet.data.dojo_populora import train_dojo_step
	...
	for episode in range(n_episodes):
		...
		for _tick in range(max_ticks):
			...
			if "born_child" in result_a and agent_d_episode is None:
				...
				# Cruzar pesos SVD
				from src.bitnet.growth.genetic import recombine_parents
		...
		from src.bitnet.growth.consolidate_sleep import consolidate_latent_resonance
		...
		if dojo_sleep_epochs > 0 and (episode + 1) % dojo_interval == 0:
			...
			from src.bitnet.data.dojo_populora import train_sequential_dojo_step
